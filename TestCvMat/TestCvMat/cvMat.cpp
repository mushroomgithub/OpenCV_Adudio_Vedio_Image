...
void PrintMat(CvMat *A);//打印显示矩阵
void GenRandn(CvMat *arr,int seed);//生成正态分布的随机矩阵
void GenRand(CvMat *arr,int seed);//生成[0,1]均匀分布的随机矩阵
static int com_func(const void *_a,const void *_b,void *userData);//比较函数
void Test_Multiply();//测试矩阵相乘
void Test_cvGetRawData();//讲缓存数据填入CvMat数组中
void Test_DCT();//计算二维离散余弦变换
